...
//Constant declaration for Zomato
//L = Link
//B = Base
//K = Key
//S = Sort
...
function getRestaurantChoices(longitude, latitude) {
  ...
  fetch(link)
    .then(r => r.json())
    .then(restLink => {
      let restFound = parseInt(restList.page.totalElements)
      if (restFound > 0) {
        let restJSON = restList._embedded.events
        restJSON.forEach(rest => {
          //console.log(event)
          listOfRest.push(new Event(rest))
        });
      } else {
        console.log('Nothing found')
      }
